...
examples.types.eq = function() {
  ...
  vg = vg.add.types(vg, typeNames=c("selfish","reciprocal"))
  vg

  tg = vg.to.tg(vg)
  tg
  utils = selfishReciprocalTypesUtil(1:2, altru=1, reci=-1)
  set.tg.util(tg,utils )

  eq.li = gtree.solve.spe(tg)
  eq.li.outcomes(eq.li)

  restore.point.options(display.restore.point = TRUE)
  eq.li.tables(eq.li,tg,ignore.keys = c("typeInd_1","typeInd_2","numPlayers"))
...
selfishReciprocalTypesUtil = function(player = 1,altru = 1, reci=-1) {
  ...
  util = lapply(player, function(i) {
    ...
    formula = substitute(payoff_i + cases(
      ...
      typeInd_i == 2 & typeInd_j==2, + (altru * payoff_i),
      0
    ),subst)
    formula
  })
